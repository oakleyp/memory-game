...
class EndScreen {
    ...
    constructor(bgcontainer = document.getElementsByTagName('body')[0], 
                 elementstofloat = [document.getElementsByTagName('button')]) {
        
        this.bgelem = bgcontainer;
        this.elems = elementstofloat;
        this.elemcount = elementstofloat.length;
    ...
    loop(angle, angles, objs, objX, objY, deg2rad, x, y, radius, radiusSat) {
        for (var i = 0; i < angles.length; i++) {
            ...
            /*if (angle > 180 && angle < 200) {
                ...
                objs[i].style.color = this.getRandomColor();
                if (objs[i].id != "logo") {
                    //Set random rgba color, alpha defaults to 0.3 if random comes up short
                    objs[i].style.backgroundColor = "rgba(" + this.getRandomColor(false) + "," + this.getRandomColor(false) + "," + this.getRandomColor(false) + ", " + (alphabit > (0.1) ? alphabit : (0.3)) + ")";
                }
            }*/
    ...
    startAnimation() {
        ...
        var centerx = Math.round(window.innerWidth * .3);
        ...
        this.loop(angle, angles, objs, objX, objY, deg2rad, x, y, radius, radiusSat);
    }
}
